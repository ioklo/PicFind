...
def main() -> None:
    ...
    with st.sidebar:
        ...
        multilingual_text = st.checkbox("한국어 직접 임베딩", value=True)
        st.markdown("인덱싱은 CLI에서 수행합니다.")
        st.code('picfind --no-caption index --source "D:\\Photos"')
    ...
    if not search_clicked:
        st.info("문장을 입력하고 Enter 또는 검색 버튼을 누르세요.")
        return
    ...
    settings = Settings(db_path=Path(db_path_input), enable_multilingual_text=multilingual_text)
    if not settings.db_path.exists():
        st.error(f"DB 파일을 찾을 수 없습니다: {settings.db_path}")
        return

    with st.spinner("검색 중..."):
        response = search_images(query.strip(), settings, limit=limit)

    st.caption(f"검색 경로: {response.embedding_mode}")

    if not response.results:
        st.warning("검색 결과가 없습니다. 먼저 인덱싱을 실행했는지 확인하세요.")
        return

    st.success(f"{len(response.results)}개의 결과를 찾았습니다.")
